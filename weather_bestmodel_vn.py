...
for name, model in models.items():
    # 1. Huấn luyện lại trên TOÀN BỘ dữ liệu (X, y)
    print(f"   -> Đang lưu: {name}...")
    model.fit(X, y)
    
    # 2. Tạo tên file an toàn (Bỏ dấu ngoặc, thay khoảng trắng bằng gạch dưới)
    safe_name = name.split("(")[-1].replace(")", "").replace(" ", "_")
    filename = f"models/model_{safe_name}.joblib"
